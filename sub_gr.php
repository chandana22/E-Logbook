...
if ($row['count(*)']==0)
   die("error");
	...
	$query = "Select count(*) from logbook_final where sub_name='Physics'";
	$s = mysqli_query($con,$query);
	$row=mysqli_fetch_array($s);
	$a1=$row['count(*)']; 

	$query = "Select count(*) from logbook_final where sub_name='Mechanical'";
	$s = mysqli_query($con,$query);
	$row=mysqli_fetch_array($s);
	$a2=$row['count(*)']; 
	$query = "Select count(*) from logbook_final where sub_name='Advanced Algorithms'";
$s = mysqli_query($con,$query);
	$row=mysqli_fetch_array($s);
	$a3=$row['count(*)']; 
		$query = "Select count(*) from logbook_final where sub_name='Data Structures'";
$s = mysqli_query($con,$query);
	$row=mysqli_fetch_array($s);
	$a4=$row['count(*)'];
	
	$query = "Select count(*) from logbook_final where sub_name='Web Programming'";
$s = mysqli_query($con,$query);
	$row=mysqli_fetch_array($s);
	$a5=$row['count(*)'];
	
	$query = "Select count(*) from logbook_final where sub_name='UNIX'";
$s = mysqli_query($con,$query);
	$row=mysqli_fetch_array($s);
	$a6=$row['count(*)'];
	
	$query = "Select count(*) from logbook_final where sub_name='C programming'";
$s = mysqli_query($con,$query);
	$row=mysqli_fetch_array($s);
	$a7=$row['count(*)'];
	
	$query = "Select count(*) from logbook_final where sub_name='Computer Networking'";
$s = mysqli_query($con,$query);
	$row=mysqli_fetch_array($s);
	$a8=$row['count(*)'];
	
	$val = array($a1,$a2,$a3,$a4,$a5,$a6,$a7,$a8);
	$grade=array('Physics','Mechanical','ADA','DSC','Web Progrm','UNIX','C program','CN');
	/* CAT:Bar Chart */
